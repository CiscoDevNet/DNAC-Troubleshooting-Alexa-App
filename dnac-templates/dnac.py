...
ENDPOINT_DEPLOY_SUMMARY ="template-programmer/template/deploy/status/%s"
RETRY_INTERVAL=2
...
def wait_on_deploy(deploy_id, token, timeout=(5*RETRY_INTERVAL), retry_interval=RETRY_INTERVAL):
    """ Waits for the specified task to complete
    """

    task_url = create_url(ENDPOINT_DEPLOY_SUMMARY % deploy_id, token["controller_ip"])
    ...
    while True:
        ...
        if "endTime" in response:
            ...
            print("deploy=%s has not completed yet. Sleeping %s seconds..." %(deploy_id, retry_interval))
            time.sleep(retry_interval)

        if response['isError'] == True:
            raise TaskError("Task %s had error %s" % (deploy_id, response['progress']))
